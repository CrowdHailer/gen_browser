defmodule GenBrowser.Mailbox do
  ...
  def read(nil, nil, supervisor, config) do
    ...
    case started do
      {:ok, pid} ->
        ...
        {:ok, [%{id: "#{mailbox_id}:0", data: %{address: address, config: config}, type: :init}]}
    end
  end
  ...
  @impl GenServer
  def handle_info({:DOWN, ref, :process, pid, reason}, state = %{clients: clients}) do
    {^pid, clients} = Map.pop(clients, ref)
    {:noreply, %{state | clients: clients}}
  end

  def handle_info(message, state = %{backlog: backlog}) do
    id = "#{state.mailbox_id}:#{length(backlog) + 1}"
